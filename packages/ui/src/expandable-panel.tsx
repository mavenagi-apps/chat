  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-shrink overflow-auto">
        <div className="flex flex-none flex-col border-r border-gray-200">
          {items.map((item) => (
            <div
              key={item.id}
              className="inline-flex h-14 w-16 items-start justify-start"
            >
              ...
              {isActiveItem(item) && (
                <div className="h-14 w-1 rounded-bl-2xl rounded-tl-2xl bg-violet-700" />
              )}
            </div>
          ))}
        </div>
        ...
        >
          <div className="h-full flex-1 p-4">
            {items.find((item) => item.id === openItem)?.content}
          </div>
        </Transition>
      </div>
      ...
      {footer && (
        <div
          className={clsx(
            "flex-0 group border-t py-4",
            openItem !== undefined ? "px-4" : "px-3",
          )}
