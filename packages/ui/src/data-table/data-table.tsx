...
export const useDataTable = <TData,>(props: UseDataTableProps<TData>) => {
  ...
  const table = useReactTable({
    ...
    columns: props.enableRowSelection
      ? rowSelectorColumn<TData>(props.enableMultiRowSelection).concat(
          props.columns,
        )
      : props.columns,
    state: {
      sorting:
        props.queryState &&
        props.queryState.sortId !== undefined &&
        props.queryState.sortDesc !== undefined
          ? [
              {
      ...
      props.setQueryState && props.queryState
        ? (updaterOrValue) => {
            ...
            props.setQueryState!((prevState) => ({
              ...
              sortDesc:
                sortingState.length > 0 ? sortingState[0].desc : undefined,
            }));
          }
        : undefined,
    pageCount: data
      ? Math.trunc((data.totalElements + data.size - 1) / data.size)
      : 0,
    manualPagination: true,
    manualSorting: true,
    enableSorting:
      props.setQueryState !== undefined && props.queryState !== undefined,
      ...
      <div className="flex flex-1 flex-col">
        ...
        {(eyebrow || showHideColumns) && (
          <div className="flex flex-row items-center justify-between border-b border-b-zinc-950/10">
            <div className="flex flex-grow flex-row items-center gap-4 px-3 py-1.5">
              {eyebrow}
            </div>
            {showHideColumns && (
        ...
        <Table>
          <colgroup>
            {table.getVisibleLeafColumns().map((column) => (
              <col
                key={column.id}
                className={(column as any).columnDef.meta?.colClassName}
              />
            ))}
          </colgroup>
          <TableHead>
                  ...
                  return (
                    <TableHeader
                      ...
                      className={cn(
                        ...
                        (header.column as any).columnDef.meta?.headerClassName,
                      )}
                  ...
                  <TableRow key={i}>
                    {table.getVisibleLeafColumns().map((column) => (
                      <TableCell
                        key={column.id}
                        className={
                          (column as any).columnDef.meta?.cellClassName
                        }
                      >
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell
                        key={cell.id}
                        className={
                          (cell.column as any).columnDef.meta?.cellClassName
                        }
                        onClick={
