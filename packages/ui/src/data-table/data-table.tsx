        ...
        <Table>
          <colgroup>
            {table.getVisibleLeafColumns().map(column => (
              <col key={column.id} className={(column as any).columnDef.meta?.colClassName} />
            ))}
          </colgroup>
          <TableHead>
          ...
          <TableBody>
            {isPending
              ? Array.from({length: table.getState().pagination.pageSize}).map((_, i) => (
                  <TableRow key={i}>
                    {table.getVisibleLeafColumns().map(column => (
                      <TableCell key={column.id} className={(column as any).columnDef.meta?.cellClassName}>
              ...
              : table.getRowModel().rows.map(row => (
                  ...
                  >
                    {row.getVisibleCells().map(cell => (
                      <TableCell
                        key={cell.id}
                        className={(cell.column as any).columnDef.meta?.cellClassName}
                        onClick={
