...
}: { anchor?: any } & Omit<HeadlessMenuItemsProps, "anchor">) {
  return (
    <HeadlessTransition
      as={Fragment}
      leave="duration-100 ease-in"
...
export function FloatingItem({
  ...
  return (
    <div
      key={key}
      className={cn("self-stretch p-1 hover:bg-gray-200", className)}
    >
...
export function DropdownItem({
  as,
  ...props
}: { href?: string; as?: React.ElementType } & Omit<
...
export function DropdownRadioItem({
  ...
  return (
    <DropdownItem
      ...
      className={cn(
        checked &&
          "data-[checked]:text-purple-700 [&>[data-slot=icon]]:data-[checked]:text-purple-700",
      )}
      {...props}
    >
      {(bag) => (
