...
export type DesktopNavbarBrandProps = React.PropsWithChildren<{href?: string; className?: string}>
export const DesktopNavbarBrand = ({className, href, ...props}: DesktopNavbarBrandProps) => {
  const Element = href ? Link : 'div'
  return (
    <Element
      // @ts-expect-error Link
      href={href}
      className={cn('text-fg-primary flex h-full flex-shrink-0 items-center px-4 text-sm font-semibold', className)}
      {...props}
    />
  )
}
