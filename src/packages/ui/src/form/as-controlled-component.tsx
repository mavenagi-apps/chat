...
export function asControlledComponent<P extends {}, DefaultValueRequired = false>(
  Component: React.ComponentType<P & {invalid?: boolean}>,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  {valueAsChecked = false, defaultValue}: {valueAsChecked?: boolean; defaultValue?: any} = {}
) {
  const AsControlledComponent = ({
    defaultValue: defaultValueProp,
    ...props
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  }: P & (DefaultValueRequired extends true ? {defaultValue: any} : {defaultValue?: any})) => {
    ...
    return (
      <>
        <Controller
          ...
          render={({field: {onChange, onBlur, value, disabled, name}, fieldState: {invalid}}) => (
            <Component
              onChange={onChange}
              onBlur={onBlur}
